...
| 依赖 | 检查命令 | 说明 |
|------|----------|------|
| Bun/Node | `bun --version` 或 `node --version` | 运行工具脚本 |
...
| 操作 | 命令 |
|------|------|
| 创建调研 | `bun run kg new-topic "主题"` 或 `node run kg new-topic "主题"` |
| 推导方向 | `bun run kg next <dir>` 或 `node run kg next <dir>` |
| 执行采集 | `bun run research <dir>` 或 `node run research <dir>` |
| 准备分析 | `bun run kg analyze <dir>` 或 `node run kg analyze <dir>` | 列出待分析页面，Agent 执行分析 |
| 查看统计 | `bun run kg stats <dir>` 或 `node run kg stats <dir>` |
| 导出 Mermaid | `bun run kg mermaid <dir>` 或 `node run kg mermaid <dir>` |
| 导出图片 | `bun run kg image <dir>` 或 `node run kg image <dir>` |
| 生成报告 | `bun run kg report <dir>` 或 `node run kg report <dir>` |
| 生成记录 | `bun run kg research-record <dir>` 或 `node run kg research-record <dir>` |
| 生成知识列表 | `bun run kg knowledge-list <dir>` 或 `node run kg knowledge-list <dir>` |
| 清理过期文件 | `bun run kg clean <dir>` 或 `node run kg clean <dir>` |
...
**Step 1: 运行 `bun run kg analyze <dir>` 或 `node run kg analyze <dir>` 查看待分析页面**

```
bun run kg analyze /path/to/topic
# 或
node run kg analyze /path/to/topic
```
...
# 或
echo '[{"label": "发现内容", "source_nodes": ["webpage_001"], "metadata": {"entities": ["实体1"], "reliability": "high"}}]' \
  | node run kg:add-findings /path/to/topic
```
